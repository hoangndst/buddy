...
def create_football_dashboard_embed():
    ...
    embed.add_field(
        name="⚽ Matches", value="See upcoming 7 days fixtures", inline=True
    )
...
class ShowMatchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=None, embed=loading_embed, view=view
        )
...
class ShowStandingButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            style=discord.ButtonStyle.success,
            custom_id="show_standing_button",
        )
        self.emoji = discord.PartialEmoji(name="📊")
...
class ReturnToFootballOptionsButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            label="← Back to Football",
            style=discord.ButtonStyle.secondary,
            custom_id="return_to_football_options",
        )
...
class ListLeaguesDropDown(discord.ui.Select):
    def __init__(self, option_type):
        ...
        placeholder = (
            "🏆 Select a league..."
            if option_type == OPTION_TYPE_STANDING
            else "⚽ Select a league..."
        )
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not selected_league:
            await interaction.response.send_message(
                "❌ No league selected.", ephemeral=True
            )
            return
        ...
        try:
            ...
            await interaction.followup.edit_message(
                interaction.message.id, content=None, embed=result, view=result_view
            )
            ...
            await interaction.followup.edit_message(
                interaction.message.id, content=None, embed=error_embed, view=error_view
            )
