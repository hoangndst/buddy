...
def create_superhero_embed(superhero_data):
    ...
    image_url = superhero_data.get('image', superhero_data.get('image', superhero_data.get('image', None)))
    
    embed = discord.Embed(
        title=f"🦸‍♂️",
        color=random.choice([0xff0000, 0x00ff00, 0x0000ff, 0xff00ff, 0xffff00, 0x00ffff])
    )
